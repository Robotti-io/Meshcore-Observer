...
test('normalizes a minimal valid serial configuration with defaults', () => {
  ...
  assert.deepEqual(config.floodAdvert, { intervalHours: 3 });
  assert.deepEqual(config.brokers, []);
  assert.deepEqual(config.bots, []);
});

test('accepts startup-only and three-to-168-hour flood advert intervals', () => {
  assert.equal(loadConfig(baseEnv({ PACKETCAPTURE_FLOOD_ADVERT_INTERVAL_HOURS: '0' })).floodAdvert.intervalHours, 0);
  assert.equal(loadConfig(baseEnv({ PACKETCAPTURE_FLOOD_ADVERT_INTERVAL_HOURS: '3' })).floodAdvert.intervalHours, 3);
  assert.equal(loadConfig(baseEnv({ PACKETCAPTURE_FLOOD_ADVERT_INTERVAL_HOURS: '168' })).floodAdvert.intervalHours, 168);
});

test('rejects flood advert intervals below three hours except zero and above 168 hours', () => {
  for (const value of ['-1', '1', '2', '169', '3.5', 'hourly']) {
    assert.throws(
